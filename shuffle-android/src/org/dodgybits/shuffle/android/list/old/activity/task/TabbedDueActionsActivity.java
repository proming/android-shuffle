...
import org.dodgybits.shuffle.android.list.old.config.DueActionsListConfig;
...
public class TabbedDueActionsActivity extends AbstractTaskListActivity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        mTabHost.addTab(createTabSpec(
                ...
                ListQuery.dueToday.name(),
        			android.R.drawable.ic_menu_day));
        mTabHost.addTab(createTabSpec(
        			R.string.week_button_title,
                ListQuery.dueNextWeek.name(),
        			android.R.drawable.ic_menu_week));
        mTabHost.addTab(createTabSpec(
        			R.string.month_button_title,
                ListQuery.dueNextMonth.name(),
        			android.R.drawable.ic_menu_month));
        mTabHost.setOnTabChangedListener(new TabHost.OnTabChangeListener() {

			public void onTabChanged(String tabId) {
				Log.d(cTag, "Switched to tab: " + tabId);
				if (tabId == null) tabId = ListQuery.dueToday.name();
				mListConfig.setMode(ListQuery.valueOf(tabId));
				updateCursor();
			}
        	
        });
        
        mListConfig.setMode(ListQuery.dueToday);
        Bundle extras = getIntent().getExtras();
        if (extras != null && extras.containsKey(DUE_MODE)) {
            mListConfig.setMode(ListQuery.valueOf(extras.getString(DUE_MODE)));
        }
    }
